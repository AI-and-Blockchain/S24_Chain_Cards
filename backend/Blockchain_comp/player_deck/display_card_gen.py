...
base = os.getcwd() + '/Blockchain_comp/player_deck/assets/'

main = Font(f'{base}Vecna.otf', size=50)
desc =  Font(f'{base}Vecna.otf', size=35)
stats = Font(f'{base}Vecna Bold.otf', size=35)
...
background = Editor(f'{base}card_bg.png')
pfp = Editor(f'{base}pfp.png').resize((804,595))
...
Class = Editor(f'{base}{(character["Class"]).lower().strip(" ")}.png').resize((100,100))
# Class = Editor(f'{(character["Class"]).lower().strip(' ')}.png').resize((100, 100))
